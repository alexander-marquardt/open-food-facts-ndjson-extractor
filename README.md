...
`data/sample-data/clean_products.ndjson` holds the first ten records a real run
wrote. It is **generated, never hand-edited** — if it disagrees with the code, the
...
The selection rule is "the first ten records the run writes" — there is no
curation step. `--report` is written outside `data/sample-data/` on purpose; only
the NDJSON belongs in the tracked directory.

### What keeps it honest

`tests/test_tracked_sample_gate.py` re-derives from the tracked sample what the
current code would produce for it, and fails when the two disagree. It reads only
the sample, `config/pricing_buckets.json` and the extractor's own functions, so
it runs in CI with no dump and no taxonomy snapshot. It covers the pricing and
margin chain end to end, the document's shape, and the rendered description tail.

It cannot cover the fields that can only be re-derived from the raw dump or the
taxonomy — titles, brands, image URLs, ingredient text, nutrition values, and the
labels and membership of `taxonomy_tags` and `category_path`. Checking those means
regenerating, and the dump is not something a GitHub runner can fetch, so that
half is a local step and not a CI one. The test module names the boundary in full.
